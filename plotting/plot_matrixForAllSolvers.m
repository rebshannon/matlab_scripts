...
load /projectnb/aeracous/REBECCA/MFC/sembian/WENO_order_study.mat
...
allData.OF = semb_spark_DOD_axisym;
allData.star = semb_spark_star_axisym;
...
for loop = 1:length(solverList)
    ...
    for probeNum = 2:length(allData.(solvName))
        ...
        % interpolate to fill in desired number of timesteps (t interpolate)
        tInterpPressure.(solvName)(:,probeNum-1) = interp1(nonDim_selectedTime.(solvName),pressure.(solvName)(:,probeNum-1),nonDim_t);
    end

    % interpolate in x position (couldn't get interp2 to work)
    for timeStep = 1:points
        fullInterp_pressure.(solvName)(timeStep,:) = interp1(nonDim_probeLocations,tInterpPressure.(solvName)(timeStep,:),nonDim_x);
    end
   ...
   plotPressure.(solvName) = fullInterp_pressure.(solvName)/p_amb;
    ...
    % make color plot
    figure
    imagesc(nonDim_x,nonDim_t,plotPressure.(solvName))
    set(gca,'YDir','normal')
    colormap(turbo)
